...
export default async (req, res) => {
    ...
    if (req.method === 'POST') {
        let newRow;
        try {
            newRow = await conn.query(`INSERT INTO recipe_steps (recipe_id, y) VALUES ('${body.recipe_id}', '${body.y}') RETURNING *`);
            ...
            console.log(err)
            res.status(500).send({error: true, message: 'Error creating step'});
            return
        }

        res.status(200).send({error: false, message: 'Step created', step: newRow.rows[0]});
    }
    if (req.method === 'DELETE') {
        try {
            await conn.query(`DELETE FROM recipe_steps WHERE id = '${body.step_id}' AND recipe_id = '${body.recipe_id}'`);
        } catch (err) {
            console.log(err)
            res.status(500).send({error: true, message: 'Error deleting step'});
            return
        }
